...
- **Apache/Nginx CLF parser** — Combined Log Format: `IP - user [timestamp] "METHOD /path HTTP/ver" status bytes "referer" "user-agent"`. Extremely common — every web server produces these. Currently falls to Plain with no structured extraction. Add `LogFormat::AccessLog`, detect via the `[dd/Mon/yyyy:HH:MM:SS tz]` timestamp bracket pattern, extract IP, method, path, status, bytes into structured fields. Status code could map to level (5xx=Error, 4xx=Warn, 3xx/2xx=Info). We already have `testdata/sample_apache.log`. Touches: `parser.rs`, `highlighter.rs`, `ui.rs`.
- **Docker JSON log wrapper** — Docker wraps actual log lines in `{"log":"the real message\n","stream":"stdout","time":"..."}`. Our JSON parser finds no `message`/`msg` key so it falls back to the raw JSON as the message. Fix: add `"log"` to the message key lookup in `parse_json_line` (and strip trailing `\n`). Tiny change, big impact for anyone tailing Docker container logs. Touches: `parser.rs`.
- **klog (Kubernetes)** — `I0115 08:30:00.000000 12345 file.go:42] actual message`. Single-letter severity prefix (`I`=Info, `W`=Warn, `E`=Error, `F`=Fatal) + compact datestamp + PID + source location. Used by all core Kubernetes components (kubelet, kube-apiserver, etcd). Currently falls to Plain, level often not detected because it's a single letter. Add `LogFormat::Klog`, detect via the `^[IWEF]\d{4} ` prefix pattern. Touches: `parser.rs`, `highlighter.rs`, `ui.rs`.
- **Log4j/Java logging** — `2024-01-15 08:30:00.123 [thread-name] LEVEL com.example.Class - message`. Thread name in brackets, fully-qualified class name. Extremely common in Java/Spring services. Currently falls to Plain — level and timestamp get detected but the thread and class are noise in the message. Add `LogFormat::Log4j`, extract thread, class, message as structured fields. Touches: `parser.rs`, `highlighter.rs`, `ui.rs`.
- **Python logging** — `2024-01-15 08:30:00,123 - module_name - LEVEL - message`. Comma in timestamp milliseconds (not dot), module name, level. Default format from Python's `logging` module. Currently falls to Plain — works OK but the `- module -` prefix clutters the message. Could extract module into extra fields. Touches: `parser.rs`.
