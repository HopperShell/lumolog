...
- ~~**Stdin follow mode**~~ — `docker logs -f | lumolog -f` streams stdin via background thread + mpsc channel. `FollowableStdinSource` dups the pipe fd before dup2 redirect, reads lines in a background thread, and feeds them to the event loop. Auto-detects log format from initial burst. Same pause/resume controls as file follow mode.
