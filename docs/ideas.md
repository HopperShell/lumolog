...
- **Auto-detect more formats** — Apache/Nginx access logs, klog (Kubernetes), Rails request logs, Python tracebacks (multi-line), Docker JSON logs. Each needs a regex in `detect_format` and a `parse_*_line` function.
